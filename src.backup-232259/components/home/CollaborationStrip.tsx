...
export const CollaborationStrip: React.FC = () => {
  return (
    <section className="relative overflow-hidden border-b border-ivory-800/80 bg-[#FAF6F0] py-6 sm:py-7 lg:py-8">
      ...
      <Container>
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-14">
          {siteConfig.collaborations.map((item) => (
            ...
            >
              ...
              <span className="font-serif text-xs sm:text-sm font-medium text-emerald-950 transition-colors group-hover:text-gold-800 text-center leading-tight">
                {item.label}
              </span>
            </div>
          ))}
        ...
        {/* Strategic Framework Line */}
        <div className="mt-5 sm:mt-6 pt-4 sm:pt-5 border-t border-gold-600/15 flex flex-wrap items-center justify-center gap-x-5 sm:gap-x-7 gap-y-1 font-label text-[0.65rem] sm:text-2xs font-semibold uppercase tracking-[0.22em] text-charcoal-500">
          <span>People</span>
          <span className="text-gold-600/50">&bull;</span>
          <span>Business</span>
          <span className="text-gold-600/50">&bull;</span>
          <span>Culture</span>
          <span className="text-gold-600/50">&bull;</span>
