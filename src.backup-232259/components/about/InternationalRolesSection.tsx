...
export const InternationalRolesSection: React.FC = () => {
  return (
    <section className="relative overflow-hidden bg-[#081D17] py-20 text-white lg:py-28">
          ...
          {aboutData.internationalRoles.map((role, idx) => {
            ...
            return (
              <motion.div
                ...
                className="group relative flex flex-col justify-between rounded-2xl border border-gold-600/35 bg-[#0A261F]/90 p-7 shadow-luxury transition-all duration-300 hover:-translate-y-1.5 hover:border-gold-400 hover:shadow-[0_24px_50px_-16px_rgba(0,0,0,0.6),0_0_24px_rgba(199,154,61,0.25)] sm:p-8 backdrop-blur-xs"
              >
                {/* Top Corner Filigree */}
                ...
                <div>
                  ...
                  <div className="mb-6 flex items-center justify-between">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gold-500/50 bg-emerald-950 shadow-md text-gold-400 transition-all duration-300 group-hover:bg-gold-500 group-hover:text-emerald-950 group-hover:scale-105">
                      <Icon className="h-6 w-6" />
                    </div>

                    <div className="flex items-center gap-1.5 rounded-full border border-gold-500/30 bg-emerald-950/80 px-3 py-1 text-2xs font-semibold text-gold-300">
                      <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
                      <span>{role.period}</span>
                    </div>
                  </div>

                  <h3 className="mb-2 font-serif text-xl font-bold text-white transition-colors group-hover:text-gold-300 leading-snug">
                    {role.title}
                  </h3>
                  ...
                  <div className="mb-6 flex items-start gap-2 rounded-xl border border-gold-500/20 bg-emerald-950/60 p-3 font-sans text-xs text-ivory-600">
                  ...
                  <div className="space-y-2.5">
                    <span className="block font-label text-2xs font-bold uppercase tracking-[0.16em] text-gold-400/80">
                      Core Mandate Responsibilities:
                    </span>
                    <ul className="space-y-2 font-sans text-xs text-ivory-700">
                      {role.responsibilities.map((resp) => (
                        <li key={resp} className="flex items-start gap-2.5">
                          <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold-400" />
