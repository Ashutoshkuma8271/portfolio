...
export const TimelineSection: React.FC = () => {
  return (
    <section className="relative border-y border-ivory-800/90 bg-[#FAF6F0] py-20 lg:py-28 overflow-hidden">
          ...
          <div className="space-y-10 lg:space-y-12">
            {aboutData.timeline.map((item, idx) => {
              ...
              return (
                ...
                >
                  {/* Glowing Node */}
                  <div className="absolute left-4 top-1.5 z-10 flex h-9 w-9 -translate-x-1/2 items-center justify-center rounded-full border-2 border-gold-500 bg-[#081D17] shadow-[0_0_12px_rgba(199,154,61,0.4)] lg:left-1/2">
                    <span className="h-2.5 w-2.5 rounded-full bg-gold-400" />
                  </div>

                  {/* Content card */}
                  <div className="ml-12 w-full lg:ml-0 lg:w-1/2 lg:px-8">
                    <div className="group relative rounded-2xl border border-gold-600/25 bg-white p-6 shadow-xs transition-all duration-300 ease-out hover:-translate-y-1 hover:border-gold-500/60 hover:shadow-[0_16px_32px_-8px_rgba(18,51,43,0.15)] sm:p-7">
                      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="rounded-full border border-emerald-900/15 bg-emerald-950/5 px-2.5 py-0.5 font-label text-2xs font-semibold text-emerald-900">
                          {item.category}
                        </span>
                      </div>

                      <h3 className="mb-1.5 font-serif text-lg font-bold text-emerald-950 transition-colors group-hover:text-gold-800 sm:text-xl">
                        {item.title}
                      </h3>

                      <div className="mb-3.5 flex flex-wrap items-center gap-x-3 gap-y-1 font-sans text-xs text-charcoal-600">
