...
import { CheckCircle2, Send, Loader2, Building, Globe, DollarSign, Mail, Phone, User } from 'lucide-react';
...
export const TradeInquiryForm: React.FC = () => {
  ...
  const onSubmit = async (data: TradeInquiryFormData) => {
    ...
    setRefId(`ZK-TRADE-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`);
    console.log('Trade inquiry logged:', data);
    setIsLoading(false);
    setIsSubmitted(true);
    reset();
  };
  ...
  if (isSubmitted) {
    return (
      <div className="bg-[#0A261F] border-2 border-gold-500/40 p-8 sm:p-10 rounded-3xl text-center text-ivory-500 shadow-luxury-lg animate-fade-in space-y-5">
        <div className="w-16 h-16 bg-gold-500/20 text-gold-400 rounded-full flex items-center justify-center mx-auto border border-gold-500/40 shadow-[0_0_20px_rgba(199,154,61,0.3)]">
          <CheckCircle2 className="w-8 h-8" />
        </div>
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-900/60 border border-emerald-500/30 text-emerald-300 font-label text-2xs font-bold uppercase tracking-wider mb-3">
            <span>Filing Reference: {refId}</span>
          </div>
          <h3 className="font-serif text-2xl sm:text-3xl font-bold text-white mb-2">
            Trade & Investment Dossier Dispatched
          </h3>
          <p className="text-sm text-ivory-600 max-w-md mx-auto leading-relaxed">
        ...
        <div className="pt-3 flex flex-wrap items-center justify-center gap-3.5">
          <a
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-gold-500 via-gold-600 to-gold-500 hover:bg-gold-400 text-emerald-950 font-bold text-xs uppercase tracking-wider rounded-full shadow-gold-glow transition-all"
          >
            Direct WhatsApp Desk
          </a>
          <button
            onClick={() => setIsSubmitted(false)}
            className="px-5 py-2.5 border border-gold-500/40 bg-emerald-950/80 text-gold-300 hover:text-white rounded-full font-label text-xs uppercase tracking-wider transition-all"
          >
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        ...
        <div>
          ...
          <div className="relative">
            <Globe className="w-4 h-4 text-gold-600 absolute left-3.5 top-3.5" />
            <select
              {...register('region')}
        ...
        <div>
          ...
          <div className="relative">
            <DollarSign className="w-4 h-4 text-gold-600 absolute left-3.5 top-3.5" />
            <select
              {...register('ticketSize')}
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-ivory-800 text-sm focus:border-gold-500 focus:ring-1 focus:ring-gold-500 outline-none transition-all bg-ivory-500/50 appearance-none"
            >
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full py-4 rounded-full bg-emerald-800 hover:bg-gold-600 hover:text-emerald-950 text-ivory-500 font-bold text-xs uppercase tracking-widest transition-all duration-300 shadow-md hover:shadow-gold-glow flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
      >
        {isLoading ? (
