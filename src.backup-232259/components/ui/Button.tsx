...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantClasses = {
    ...
    secondary:
      'border border-emerald-800/40 bg-transparent text-emerald-900 hover:border-emerald-800 hover:bg-emerald-800 hover:text-ivory-500',
    gold:
      'border border-gold-500 bg-gold-600 font-bold text-emerald-950 shadow-gold-glow hover:bg-gold-500 hover:shadow-lg',
    tertiary:
      'border border-transparent bg-transparent text-emerald-950 underline-offset-4 hover:text-gold-700 hover:underline',
    ghost:
      'border border-transparent bg-transparent text-emerald-900 hover:bg-emerald-800/10',
  }[variant];
  ...
  const baseStyles = [
    'inline-flex items-center justify-center gap-2.5 rounded-full font-sans font-semibold tracking-wider',
  ...
  const content = (
    <>
      ...
      <span className={uppercase ? 'uppercase' : undefined}>{children}</span>
      {showArrow && !loading && (
